  ...
  return (
    ...
    >
      <div className="flex items-center justify-between w-full px-4 md:px-8 lg:px-20 sm:justify-center lg:justify-between">
        {/* Logo */}
        <a href="#section-1">
          <Image src="/logoTegbe.svg" alt="Logo Tegbe" width={140} height={60} />
        </a>
      ...
      {menuOpen && (
        <div className="absolute top-20 left-0 w-full bg-black text-white flex flex-col items-center gap-6 py-6 px-4 md:hidden z-[999999]">
          <a href="#section-text-image" className="hover:underline" onClick={() => setMenuOpen(false)}>Problemas</a>
          <a href="#section-strategic-digital-transformation" className="hover:underline" onClick={() => setMenuOpen(false)}>Transformação</a>
          <a href="#section-customers" className="hover:underline" onClick={() => setMenuOpen(false)}>Parceiros</a>
          <Link
            href="#section-2"
